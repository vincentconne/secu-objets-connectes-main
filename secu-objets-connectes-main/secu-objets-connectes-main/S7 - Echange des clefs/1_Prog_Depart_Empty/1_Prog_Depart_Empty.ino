...
uint8_t A_private_key[32];
uint8_t A_public_key[32];
...
void BrokerConnect()
{
  ...
  client.subscribe("/ISIMA/S7_DH/GROUPE_03/PublicKeyServer", MQTT::QOS0, CallBackBpMsg);
}
...
// AES-CBC Cipher
char * Base64encrypt(char * plainStr)
{
  int key_size = 32;
  int encodedLength = base64_enc_len(key_size);
  char encodedKey[encodedLength];
  base64_encode(encodedKey, plainStr, key_size);
...
void setup()
{
  ...
  Curve25519::dh1(A_public_key, A_private_key);
}
...
void loop()
{
  ...
  if (Bp_S2 != digitalRead(bouton_S2))
  {
    ...
    char* payload = Base64encrypt((char *)A_public_key);
    message.payload = (void *)(payload);
    message.payloadlen = strlen(payload);
    int err = client.publish("/ISIMA/S7_DH/GROUPE_03/PublicKeyIoT", message);
#ifdef MY_DEBUG
